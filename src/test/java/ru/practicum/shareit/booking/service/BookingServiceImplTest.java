...
class BookingServiceImplTest {
    ...
    @Test
    void findAllBookingsByUser_whenFromIsNegative_thenReturnBookingDtoCollection() {
        long userId = 1L;
        String status = "ALL";
        int from = -5;
        int size = 1;

        assertThrows(ValidationException.class, () -> bookingService.findAllBookingsByUser(userId, status,
                from, size));
    }
    ...
    @Test
    void findAllBookingsByOwner_whenSizeIsZero_thenReturnBookingDtoCollection() {
        long ownerId = 1L;
        String status = "ALL";
        int from = 0;
        int size = 0;

        assertThrows(ValidationException.class, () -> bookingService.findAllBookingsByOwner(ownerId, status,
                from, size));
    }
}